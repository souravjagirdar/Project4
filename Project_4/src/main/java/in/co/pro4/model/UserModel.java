...
public class UserModel {
	...
	public static long add(UserBean bean) throws ApplicationException, DuplicateRecordException {
		...
		System.out.println("user model ki find by login me jayega");
		UserBean beanExists = findByLogin(bean.getLogIn());

		if (beanExists != null) {
			System.out.println("bean exists hua to exception throw krega");
			throw new DuplicateRecordException("LogIn ID is already Exists");
		}
		try {
			System.out.println("add or find by login ke bad yha ayege");
			conn = JDBCDataSource.getConnection();
			pk = nextPK();
		...
		System.out.println("return pk krega userregistration ctl pr");
		return pk;
	}
	/**
	 * 
	...
	public static UserBean findByLogin(String login) throws ApplicationException {
		...
		System.out.println("find by login me query check kregi");
		...
		try {
			System.out.println("try me ayi 1");
			conn = JDBCDataSource.getConnection();
			PreparedStatement pstmt = conn.prepareStatement(sql.toString());
              System.out.println("try me ayi 2");
			pstmt.setString(1, login);
			ResultSet rs = pstmt.executeQuery();

			while (rs.next()) {
				System.out.println("while method chli");
				bean = new UserBean();
		...
		System.out.println("return bean krra");
		return bean;
	}
	/**
	 * 
	...
	public List search(UserBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		System.out.println("model ki serch");
		StringBuffer sql = new StringBuffer("SELECT * FROM ST_USER WHERE 1=1");
        System.out.println("stringbuffer me query chli");
        
		if (bean != null) {
			System.out.println("bean !=null me aya condition check krega");
			if (bean.getId() > 0) {
				System.out.println("bean !=null me aya condition check krega");
		...
		if (pageSize > 0) {
			System.out.println("pagination perform krega");
		...
		try {
			conn = JDBCDataSource.getConnection();
			PreparedStatement pstmt = conn.prepareStatement(sql.toString());
             System.out.println("connection get get kiya");
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				bean = new UserBean();
                System.out.println("while ki rs.next m bean.setid,fnsame.lname.....set kra or list.add me bean ko pass kiya");
				bean.setId(rs.getLong(1));
				bean.setFirstName(rs.getString(2));
				bean.setLastName(rs.getString(3));
				bean.setLogIn(rs.getString(4));
	...
	public long registerUser(UserBean bean) throws ApplicationException, DuplicateRecordException {
		...
		System.out.println("pk ki add method me jayega");
		long pk = add(bean);
